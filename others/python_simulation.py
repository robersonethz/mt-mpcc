...
for i in range(100):  # was 100
    ...
    for j in range(model.N):
        ...
        idx = 0
        temp = 100
        while (abs(arcLength[idx] - f_theta[j]) < temp):
            temp = abs(arcLength[idx] - f_theta[j])
            idx = idx + 1
        ...
        # print(idx)
        problem['x0'][model.nvar*(j)+8] = f_theta[j]
...
# z = [0: x, 1: y, 2: yaw, 3: vx, 4: vy, 5: dyaw, 6: torque, 7: steer, 8: Theta, 9: dtorque, 10: dsteer, 11: dtheta]
    ...
    problem['xinit'] = next_state[0:9]
    # TODO Check for dimensions...
    problem['x0'] = np.tile(next_state, model.N)
    ...
    for j in range(model.N):  # iterate over all stages and get closest arc length
        ...
        if reset:
            f_theta[j] = np.mod(f_theta[j], max(arcLength)/2)
