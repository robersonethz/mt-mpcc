...
classdef FORCESNLPsolverBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x01 = coder.nullcopy(zeros(28, 1));
            outputs_x02 = coder.nullcopy(zeros(28, 1));
            outputs_x03 = coder.nullcopy(zeros(28, 1));
            outputs_x04 = coder.nullcopy(zeros(28, 1));
            outputs_x05 = coder.nullcopy(zeros(28, 1));
            outputs_x06 = coder.nullcopy(zeros(28, 1));
            outputs_x07 = coder.nullcopy(zeros(28, 1));
            outputs_x08 = coder.nullcopy(zeros(28, 1));
            outputs_x09 = coder.nullcopy(zeros(28, 1));
            outputs_x10 = coder.nullcopy(zeros(28, 1));
            outputs_x11 = coder.nullcopy(zeros(28, 1));
            outputs_x12 = coder.nullcopy(zeros(28, 1));
            outputs_x13 = coder.nullcopy(zeros(28, 1));
            outputs_x14 = coder.nullcopy(zeros(28, 1));
            outputs_x15 = coder.nullcopy(zeros(28, 1));
            outputs_x16 = coder.nullcopy(zeros(28, 1));
            outputs_x17 = coder.nullcopy(zeros(28, 1));
            outputs_x18 = coder.nullcopy(zeros(28, 1));
            outputs_x19 = coder.nullcopy(zeros(28, 1));
            outputs_x20 = coder.nullcopy(zeros(28, 1));
            outputs_x21 = coder.nullcopy(zeros(28, 1));
            outputs_x22 = coder.nullcopy(zeros(28, 1));
            outputs_x23 = coder.nullcopy(zeros(28, 1));
            outputs_x24 = coder.nullcopy(zeros(28, 1));
            outputs_x25 = coder.nullcopy(zeros(28, 1));
            outputs_x26 = coder.nullcopy(zeros(28, 1));
            outputs_x27 = coder.nullcopy(zeros(28, 1));
            outputs_x28 = coder.nullcopy(zeros(28, 1));
            outputs_x29 = coder.nullcopy(zeros(28, 1));
            outputs_x30 = coder.nullcopy(zeros(28, 1));
            outputs_x31 = coder.nullcopy(zeros(28, 1));
            outputs_x32 = coder.nullcopy(zeros(28, 1));
            outputs_x33 = coder.nullcopy(zeros(28, 1));
            outputs_x34 = coder.nullcopy(zeros(28, 1));
            outputs_x35 = coder.nullcopy(zeros(28, 1));
            outputs_x36 = coder.nullcopy(zeros(28, 1));
            outputs_x37 = coder.nullcopy(zeros(28, 1));
            outputs_x38 = coder.nullcopy(zeros(28, 1));
            outputs_x39 = coder.nullcopy(zeros(28, 1));
            outputs_x40 = coder.nullcopy(zeros(28, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = FORCESNLPsolverBuildable.forcesCall(params.xinit, params.x0, params.all_parameters, params.num_of_threads, params.receive_floating_license);
        end

        function [output,exitflag,info] = forcesCall(xinit, x0, all_parameters, num_of_threads, receive_floating_license)
            ...
            params = struct('xinit', double(xinit),...
                            ...
                            'receive_floating_license', int32(receive_floating_license));
            ...
            output.x26 = cast(output_c.x26, 'like', output.x26);
            output.x27 = cast(output_c.x27, 'like', output.x27);
            output.x28 = cast(output_c.x28, 'like', output.x28);
            output.x29 = cast(output_c.x29, 'like', output.x29);
            output.x30 = cast(output_c.x30, 'like', output.x30);
            output.x31 = cast(output_c.x31, 'like', output.x31);
            output.x32 = cast(output_c.x32, 'like', output.x32);
            output.x33 = cast(output_c.x33, 'like', output.x33);
            output.x34 = cast(output_c.x34, 'like', output.x34);
            output.x35 = cast(output_c.x35, 'like', output.x35);
            output.x36 = cast(output_c.x36, 'like', output.x36);
            output.x37 = cast(output_c.x37, 'like', output.x37);
            output.x38 = cast(output_c.x38, 'like', output.x38);
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x01 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x02 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x03 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x04 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x05 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x06 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x07 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x08 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x09 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x10 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x11 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x12 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x13 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x14 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x15 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x16 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x17 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x18 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x19 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x20 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x21 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x22 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x23 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x24 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x25 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x26 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x27 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x28 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x29 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x30 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x31 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x32 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x33 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x34 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x35 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x36 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x37 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x38 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x39 = coder.nullcopy(double(zeros(28, 1)));
            outputs_x40 = coder.nullcopy(double(zeros(28, 1)));
            output = struct('x01', outputs_x01,...
                            'x02', outputs_x02,...
                            'x03', outputs_x03,...
                            'x04', outputs_x04,...
