...
classdef FORCESNLPsolverBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_x01 = coder.nullcopy(zeros(16, 1));
            outputs_x02 = coder.nullcopy(zeros(16, 1));
            outputs_x03 = coder.nullcopy(zeros(16, 1));
            outputs_x04 = coder.nullcopy(zeros(16, 1));
            outputs_x05 = coder.nullcopy(zeros(16, 1));
            outputs_x06 = coder.nullcopy(zeros(16, 1));
            outputs_x07 = coder.nullcopy(zeros(16, 1));
            outputs_x08 = coder.nullcopy(zeros(16, 1));
            outputs_x09 = coder.nullcopy(zeros(16, 1));
            outputs_x10 = coder.nullcopy(zeros(16, 1));
            outputs_x11 = coder.nullcopy(zeros(16, 1));
            outputs_x12 = coder.nullcopy(zeros(16, 1));
            outputs_x13 = coder.nullcopy(zeros(16, 1));
            outputs_x14 = coder.nullcopy(zeros(16, 1));
            outputs_x15 = coder.nullcopy(zeros(16, 1));
            outputs_x16 = coder.nullcopy(zeros(16, 1));
            outputs_x17 = coder.nullcopy(zeros(16, 1));
            outputs_x18 = coder.nullcopy(zeros(16, 1));
            outputs_x19 = coder.nullcopy(zeros(16, 1));
            outputs_x20 = coder.nullcopy(zeros(16, 1));
            outputs_x21 = coder.nullcopy(zeros(16, 1));
            outputs_x22 = coder.nullcopy(zeros(16, 1));
            outputs_x23 = coder.nullcopy(zeros(16, 1));
            outputs_x24 = coder.nullcopy(zeros(16, 1));
            outputs_x25 = coder.nullcopy(zeros(16, 1));
            outputs_x26 = coder.nullcopy(zeros(16, 1));
            outputs_x27 = coder.nullcopy(zeros(16, 1));
            outputs_x28 = coder.nullcopy(zeros(16, 1));
            outputs_x29 = coder.nullcopy(zeros(16, 1));
            outputs_x30 = coder.nullcopy(zeros(16, 1));
            outputs_x31 = coder.nullcopy(zeros(16, 1));
            outputs_x32 = coder.nullcopy(zeros(16, 1));
            outputs_x33 = coder.nullcopy(zeros(16, 1));
            outputs_x34 = coder.nullcopy(zeros(16, 1));
            outputs_x35 = coder.nullcopy(zeros(16, 1));
            outputs_x36 = coder.nullcopy(zeros(16, 1));
            outputs_x37 = coder.nullcopy(zeros(16, 1));
            outputs_x38 = coder.nullcopy(zeros(16, 1));
            outputs_x39 = coder.nullcopy(zeros(16, 1));
            outputs_x40 = coder.nullcopy(zeros(16, 1));
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_x01 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x02 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x03 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x04 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x05 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x06 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x07 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x08 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x09 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x10 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x11 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x12 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x13 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x14 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x15 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x16 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x17 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x18 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x19 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x20 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x21 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x22 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x23 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x24 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x25 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x26 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x27 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x28 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x29 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x30 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x31 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x32 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x33 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x34 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x35 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x36 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x37 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x38 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x39 = coder.nullcopy(double(zeros(16, 1)));
            outputs_x40 = coder.nullcopy(double(zeros(16, 1)));
            output = struct('x01', outputs_x01,...
                            'x02', outputs_x02,...
                            'x03', outputs_x03,...
                            'x04', outputs_x04,...
